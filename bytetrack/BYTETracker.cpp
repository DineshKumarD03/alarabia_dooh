...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
    ...
    {
        const auto strack = std::make_shared<STrack>(object.rect, object.prob, object.class_id);
...

    {
        ...
        for (const auto &unmatch_track : unmatch_track_idx)
        {
            ...
            {
                // auto track = std::make_shared<STrack>(track->rect_, track->prob_, track->getClassId());
                track->markAsLost();
